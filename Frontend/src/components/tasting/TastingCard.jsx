...
const TastingCard = ({ tasting, whiskeyName, onEdit, onDelete }) => {
  const {
    id,
  ...
  if (!id) {
    return (
      <Card
        className="p-4 text-center text-red-600 dark:text-red-400 bg-red-50 dark:bg-gray-800 border border-red-200 dark:border-red-700"
        dir="rtl"
      >
  ...
  return (
    <Card
      className="flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300 dark:bg-gray-800"
      dir="rtl"
    >
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start">
          ...
          {rating > 0 && (
            <div
              className="flex items-center gap-1 text-yellow-500 dark:text-yellow-400"
              title={`דירוג: ${rating} כוכבים`}
            >
              {[...Array(5)].map((_, i) => {
                let starClassName = 'stroke-current opacity-50'; // Default
                ...
                return <Star key={i} size={18} className={starClassName} />;
              })}
              <span className="text-xs ml-1 rtl:mr-1">
                ({rating.toFixed(1)})
              </span>
            </div>
          )}
      ...
      <Link to={detailPagePath} className="block group">
        <div className="h-40 w-full bg-gray-100 dark:bg-gray-700/50 flex items-center justify-center overflow-hidden relative group-hover:opacity-90 transition-opacity mx-4 rounded-md">
          ...
          <div
            className={`placeholder-icon w-full h-full flex-col items-center justify-center text-gray-400 dark:text-gray-500 ${imageUrl ? 'hidden' : 'flex'}`}
            style={{ display: imageUrl ? 'none' : 'flex' }}
          >
            <ImageOff size={48} />
            <p className="text-xs mt-1">אין תמונה</p>
          </div>
          {imageUrl && ( // Eye icon for hover effect
          ...
          {glassware && (
            ...
            >
              ...
              >
                ...
                <path d="M5.52 4.22a7 7 0 0 1 12.96 0" />
              </svg>{' '}
              {glassware}
            </Badge>
          )}
