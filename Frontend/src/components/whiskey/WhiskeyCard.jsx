...
const WhiskeyCard = ({ whiskey, onEdit, onDelete }) => {
  ...
  if (!id) {
    ...
    return (
      <Card
        className="flex flex-col items-center justify-center h-full shadow-lg p-4 dark:bg-gray-800 text-center"
        dir="rtl"
      >
        <Wine className="w-16 h-16 text-red-300 dark:text-red-500 mb-2" />
        <p className="text-red-600 dark:text-red-400">
          שגיאה: נתוני וויסקי חסרים או לא תקינים.
        </p>
      </Card>
    );
  }

  // Ensure bottleStatus is a number for width calculation and display
  let numericBottleStatus = null;
  let displayBottleStatus = 'לא ידוע';

  if (typeof bottleStatus === 'number') {
    numericBottleStatus = Math.max(0, Math.min(100, bottleStatus)); // Clamp between 0 and 100
    displayBottleStatus = `${numericBottleStatus}%`;
  } else if (typeof bottleStatus === 'string') {
    const parsed = parseFloat(bottleStatus.replace('%', ''));
  ...
  return (
    <Card
      className="flex flex-col overflow-hidden h-full shadow-lg hover:shadow-xl transition-shadow duration-300 dark:bg-gray-800"
      dir="rtl"
    >
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start">
          ...
          {isFavorite && (
            <StarIcon
              className="w-5 h-5 text-yellow-400 fill-yellow-400 flex-shrink-0"
              title="מועדף"
            />
          )}
        </div>
      ...
      <Link to={detailPagePath} className="block group flex-grow">
        <div className="h-48 w-full bg-amber-50 dark:bg-gray-700/50 rounded-md flex items-center justify-center overflow-hidden relative group-hover:opacity-80 transition-opacity mx-4">
          {imageUrl ? (
            <img
              ...
              onError={(e) => {
                ...
                const placeholder =
                  e.target.parentElement.querySelector('.placeholder-icon');
                if (placeholder) {
          ...
          {/* Placeholder Icon - shown if imageUrl is missing OR if image fails to load */}
          <div
            className={`placeholder-icon w-full h-full items-center justify-center ${imageUrl ? 'hidden' : 'flex'}`}
            style={{ display: imageUrl ? 'none' : 'flex' }}
          >
            <Wine className="w-20 h-20 text-amber-300 dark:text-amber-500" />
          </div>

          {imageUrl && ( // Show eye icon for hover effect only if image exists
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {age != null && age !== '' && (
            <Badge variant="outline">
              {Number(age) === 0 ? 'NAS' : `${age} שנים`}
            </Badge>
          )}
          {abv != null && abv !== '' && (
            <Badge
              variant="outline"
              className="bg-red-50 border-red-200 text-red-700 dark:bg-red-900/30 dark:border-red-700 dark:text-red-300"
            >
              {abv}% ABV
            </Badge>
          )}
        ...
        {numericBottleStatus !== null && (
          <div className="pt-1">
            <div className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex items-center">
              <Percent size={14} className="ml-1 rtl:mr-1 rtl:ml-0" /> כמות
              נותרת: {displayBottleStatus}
            </div>
