    ...
    try:
        ...
        if facts_section:
            ...
            if fact_items:
                ...
                fact_items = [line.strip() for line in facts_text.split('\n') if line.strip()]
                if fact_items:
                    facts = fact_items
...
def analyze_collection(whiskeys: List[Whiskey], tastings: List[Tasting]) -> Dict[str, Any]:
    """Analyze a user's whiskey collection and tastings"""
    try:
        if not whiskeys:
            return {
                "analysis": "אין מספיק נתונים לניתוח - הוסף ויסקי לאוסף שלך כדי לקבל תובנות.",
                "recommendations": [],
        ...
        for whiskey in whiskeys:
            if whiskey.id in whiskey_ratings and whiskey_ratings[whiskey.id]["count"] > 0:
                avg = whiskey_ratings[whiskey.id]["sum"] / whiskey_ratings[whiskey.id]["count"]
        ...
        data = {
            ...
            "messages": [
                ...
                סוגי ויסקי באוסף: {', '.join(set(whiskey_types)) if whiskey_types else "אין מידע"} 
                אזורים באוסף: {', '.join(set(whiskey_regions)) if whiskey_regions else "אין מידע"}
                ויסקי מדורגים בראש: {', '.join(top_whiskeys_info) if top_whiskeys_info else "אין דירוגים"}
...
def recommend_whiskey(whiskeys: List[Whiskey], tastings: List[Tasting]) -> Dict[str, Any]:
