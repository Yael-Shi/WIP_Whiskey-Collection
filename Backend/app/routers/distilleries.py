...
from app.schemas.distillery_schema import Distillery as DistillerySchema, DistilleryCreate, DistilleryUpdate
from app.services import distillery_service
from app.auth.auth import get_current_active_user # For protecting routes
# from app.auth.auth import get_current_active_superuser # If some actions are admin-only

router = APIRouter()

@router.post("/distilleries/", response_model=DistillerySchema, status_code=status.HTTP_201_CREATED)
def create_new_distillery(
    ...
    """
    db_distillery_by_name = distillery_service.get_distillery_by_name(db, name=distillery.name)
    if db_distillery_by_name:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Distillery with name '{distillery.name}' already exists.")
...
def update_existing_distillery(
    ...
    """
    updated_distillery = distillery_service.update_distillery(db, distillery_id=distillery_id, distillery_update_data=distillery_update)
...
def delete_existing_distillery(
    ...
    """
    deleted_distillery = distillery_service.delete_distillery(db, distillery_id=distillery_id)
